...
def generate_prefix(s3_bucket_prefix, start_date):
    return os.path.join(
        s3_bucket_prefix, start_date.strftime("%Y{0}%m{0}%d").format(os.path.sep)
    )
...
def check_export_status(task_id, delay=1):
    ...
    try:
        ...
        # We want to be sure we're not beating up the API and
        # we'll increase our delay until the task completes
        while response == "RUNNING":
            logging.warning(
                "We did not get a COMPLETED response so waiting "
                + str(delay)
                + " seconds until checking status again"
            )
            time.sleep(delay)
            response = export_status_call(task_id=task_id)
            delay = delay * 2
